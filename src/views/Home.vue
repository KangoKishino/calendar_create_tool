            ...
            <table id="main-table">
              <thead>
                <tr>
                    ...
                    <th id="title" colspan="5">{{ this.$store.getters.year }}月{{ this.$store.getters.month + 1 }}日</th>
        ...
        <div class="boxContainer group d-flex justify-content-around">
          <div id="detail" class="box">
              <table id="coloring">
                ...
                <tr>
                  <th>
                    <div class="pointer" @click="changePallet1">カラー1</div>
                    <MyModal @close="changePallet1" v-show="showPallet1">
                      <chrome-picker :value="this.$store.getters.detailedSettings.color1" class="modal-window" @input="updateValue"></chrome-picker>
                    </MyModal>
                  </th>
                  <th class="pallet1 pointer" :style="styles" @click="changePallet1"></th>
                </tr>
                <tr>
                  <th>
                    <div class="pointer" @click="changePallet2">カラー2</div>
                    <MyModal @close="changePallet2" v-show="showPallet2">
                      <chrome-picker :value="this.$store.getters.detailedSettings.color2" class="modal-window" @input="updateValue2"></chrome-picker>
                    </MyModal>
                  </th>
                <th class="pallet2 pointer" :style="styles2" @click="changePallet2"></th>
                </tr>
                <tr>
                  <th><div class="pointer" @click="changePallet3">背景</div>
                  <MyModal @close="changePallet3" v-show="showPallet3">
                      <chrome-picker :value="this.$store.getters.detailedSettings.color3" class="modal-window" @input="updateValue3"></chrome-picker>
                  </MyModal>
                  </th>
                <th class="pallet3 pointer" :style="styles3" @click="changePallet3"></th>
                </tr>
                <tr>
                  <th><div class="pointer" @click="changePallet4">枠線</div>
                  <MyModal @close="changePallet4" v-show="showPallet4">
                      <chrome-picker :value="this.$store.getters.detailedSettings.color4" class="modal-window" @input="updateValue4"></chrome-picker>
                  </MyModal>
                  </th>
                  <th class="pallet4 pointer" :style="styles4" @click="changePallet4"></th>
                </tr>
              </table>
            </div>
            <div class="box">
                <table id="cell-size">
                  <tr>
                    <th colspan="2">1マスの大きさ</th>
                  </tr>
                  <tr>
                      <th class="fixed-width">横</th>
                      <th><input type="number" id="input-width" v-model="width">px</th>
                  </tr>
                  <tr>
                      <th>縦</th>
                      <th><input type="number" id="input-height" v-model="height">px</th>
                  </tr>
                </table>
            </div>
            <div class="box">
                <table id="border-size"><tr>
                    <th colspan="2">枠線の太さ</th>
                  </tr><tr>
                        <th class="fixed-width"></th>
                        <th><input type="number" id="input-border" v-model="border">px</th>
                    </tr>
                </table>
            </div>
        </div>
        <div id="output-button" @click='outputDate' class="btn btn-outline-dark mt-5"><ChevronRight />Convert</div>
      </div>
      <div class="usu">
        <p class="text-left slide-right mt-3">Output</p>
        <div class="boxContainer">
          <div class="box">
            <div id="output"><ChevronRight @click='outputDate' class="side-space pointer"/>
              <textarea cols="50" rows="10" v-model="output"></textarea>
            </div>
            <button id="clip-board" class="btn btn-outline-dark slide-right"
            v-clipboard:copy="output"
            v-clipboard:error="onError"><ContentCopy />Copy</button>
          </div>
        </div>
        <div id="preview" class="d-flex justify-content-around slide-right collapse" v-html="this.$store.getters.outputDates"></div>
      </div>
    </div>
  </div>
</template>
